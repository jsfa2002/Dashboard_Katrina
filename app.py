...
with T5:
    if "mes_nombre" not in df_fe.columns or df_fe.empty:
        ...
        with ia1:
            ...
            # Preparar datos base (ventas mensuales)
            ...
            # --- Regresión Polinomial (base) ---
            if modelo_pred == "Regresión Polinomial (base)":
                X = serie["t"].values.reshape(-1, 1)
                y = serie["ventas"].values
            ...
            # --- XGBoost con Optuna ---
                ...
                # Crear características (lags y medias móviles)
                df_feat = serie.copy()
                for lag in [1, 2, 3]:
                    df_feat[f'lag_{lag}'] = df_feat['ventas'].shift(lag)
                for w in [2, 3]:
                ...
                if len(df_feat) < 4:
                    st.warning("Se requieren al menos 4 meses para usar XGBoost. Cambia a otro modelo.")
                    st.stop()
                ...
                last_row = df_feat.iloc[-1:][feature_cols].copy()
                preds = []
                for _ in range(horizonte):
                    pred = best_model.predict(last_row)[0]
                    preds.append(pred)
                    # Actualizar características para siguiente paso (simplificado)
                    ...
                    new_row['rm_2'] = (new_row['lag_1'] + new_row['lag_2'])/2
                    new_row['rs_2'] = abs(new_row['lag_1'] - new_row['lag_2'])/2
                    new_row['rm_3'] = (new_row['lag_1'] + new_row['lag_2'] + new_row['lag_3'])/3
                    new_row['rs_3'] = np.std([new_row['lag_1'].values[0], new_row['lag_2'].values[0], new_row['lag_3'].values[0]])
                    last_row = new_row
            ...
            for i, col in enumerate(cols_pred):
                with col:
                    st.metric(
                        f"📅 {mf[i]}",
                        fmt(pf[i]),
                        delta=f"{((pf[i]/serie['ventas'].iloc[-1])-1)*100:.1f}% vs {om_ia[-1]}" if i==0 else None
                    )

            # Gráfico comparativo
            fig_pred = go.Figure()
            fig_pred.add_trace(go.Scatter(
                x=serie["mes"], y=serie["ventas"],
                name="Real", mode="lines+markers",
                line=dict(color=N(GOLD), width=3), marker=dict(size=9)
            ))
            fig_pred.add_trace(go.Scatter(
                x=mf, y=pf, name="Predicción",
                mode="lines+markers", line=dict(color=N(GREEN), width=3, dash="dash"),
                marker=dict(size=10, symbol="diamond")
            ))
            fig_pred.add_trace(go.Scatter(
                x=mf+mf[::-1], y=list(ic_hi)+list(ic_lo[::-1]),
            ...
            fig_pred.add_annotation(
                x=mf[0], y=float(serie["ventas"].max())*1.05,
                text="Proyección →", showarrow=False,
                font=dict(color=N(GREEN), size=11)
            )
            fl(fig_pred, f"Predicción - {modelo_pred} - Próximos {horizonte} meses", 420)
            fig_pred.update_yaxes(tickformat=",.0f", tickprefix="$")
            ...
            df_pred = pd.DataFrame({
                ...
                "Δ vs último mes": [f"{((pf[i]/serie['ventas'].iloc[-1])-1)*100:.1f}%" for i in range(horizonte)]
            })
            ...
            Xp = sp["t"].values.reshape(-1,1)
            yp = sp["pedidos"].values
            grado_p = 2 if nm>=4 else 1
            model_p = make_pipeline(PolynomialFeatures(grado_p), LinearRegression())
            model_p.fit(Xp, yp)
            tf = np.arange(nm, nm+horizonte).reshape(-1,1)
            pp = np.maximum(model_p.predict(tf), 0).astype(int)
            texto_pedidos = " · ".join([f"<b>{pp[i]}</b> ({mf[i]})" for i in range(horizonte)])
            st.markdown(f"<p style='color:{N(GRAY)};font-size:12px;'>📦 Pedidos estimados (regresión polinomial): {texto_pedidos}</p>", unsafe_allow_html=True)
